...
public class ChoppaConversionModel implements IOverrideModel {
    public ChoppaConversionModel() {
        super();
    }

    public void render(float partialTicks, ItemDisplayContext transformType, ItemStack stack, ItemStack parent, LivingEntity entity, PoseStack matrixStack, MultiBufferSource buffer, int light, int overlay) {
        RenderUtil.renderModel(SpecialModels.CHOPPA_CONVERSION_MAIN.getModel(), stack, matrixStack, buffer, light, overlay);
        ...
        if (Gun.hasAttachmentEquipped(stack, Type.MAGAZINE)) {
            if (Gun.getAttachment(Type.MAGAZINE, stack).getItem() == ModItems.EXTENDED_MAG.get()) {
                RenderUtil.renderModel(SpecialModels.CHOPPA_CONVERSION_EXT_MAG.getModel(), stack, matrixStack, buffer, light, overlay);
            }

            if (Gun.getAttachment(Type.MAGAZINE, stack).getItem() == ModItems.SPEED_MAG.get()) {
                RenderUtil.renderModel(SpecialModels.CHOPPA_CONVERSION_SPEED_MAG.getModel(), stack, matrixStack, buffer, light, overlay);
            }
            ...
            RenderUtil.renderModel(SpecialModels.CHOPPA_CONVERSION_STAN_MAG.getModel(), stack, matrixStack, buffer, light, overlay);
        }
